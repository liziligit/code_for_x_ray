...
const int nFrame = 757;//758,26 for 270592byte=271kb, 27 for 280960byte=281kb
const int nChannel = 1;
const int nXpixel = 72;
const int nYpixel = 72;
const int nLength = (nXpixel*nYpixel)*nFrame*nChannel;
const int channel = 0;
    ...
    for(int i=0; i<(int)pdList.size(); i++)
    {
       ...
       // }
        ...
        if(ifSignal.good())
        {
            // read data
            //
            ifSignal.seekg( (-1) * nLength * sizeof(unsigned short), ios::end);
            ...
            for(int ii = 0; ii < nFrame; ii++)
            {
                ...
                {
                    oftest.write((char *)(&data[ (jj + ii*(nXpixel*nYpixel))* nChannel ]), sizeof(unsigned short));
                }
            }
