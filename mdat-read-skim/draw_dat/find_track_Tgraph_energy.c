//运行方法
// run ./energy.sh iStart_num iAccout, such as, ./energy.sh 4 3
// will get the output file energy.dat for plot energy spectrum
...
// void find_track_Tgraph_energy()
void find_track_Tgraph_energy(int iStart_num, int iAccout)
{
    ...
    for (int k = 0; k < iFrames * iAccout; k++)
    {
        if (k > 2 && k < iFrames * iAccout - 3)
        {
            ...
            // aFrames_one_mdat[k - 3] < aFrames_one_mdat[k + 2] &&)
            {
                ...
                energy << "out" << " " << k/808 << " " << "frame" << " " << k%808 << " " << aFrames_one_mdat[k] << endl;
            }
        }
    }
